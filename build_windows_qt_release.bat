...
echo [3/3] Building full Windows release package...
".venv\Scripts\python.exe" build_windows_qt_release.py --channel stable
if errorlevel 1 goto failed

echo.
echo Full release generated under:
echo %cd%\releases\windows
echo.
start "" "%cd%\releases\windows"
pause
exit /b 0

:failed
echo.
echo Full release build failed. Please check the error messages above.
pause
exit /b 1
