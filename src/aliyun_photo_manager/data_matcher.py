...
def _read_sheet_matrix(file_path: Path) -> List[List[str]]:
    ...
    if suffix == ".xls":
        ...
        for row_index in range(sheet.nrows):
            matrix.append([_normalize(sheet.cell_value(row_index, col)) for col in range(sheet.ncols)])
        return matrix
...
def _extract_headers_and_rows(matrix: List[List[str]]) -> Tuple[int, List[str], List[List[str]]]:
    ...
    for index, header in enumerate(raw_headers):
        normalized = header or f"未命名列{index + 1}"
...
def run_data_match(options: DataMatchOptions, logger: LogFn = None) -> DataMatchSummary:
    ...
    _log(
        logger,
        f"目标表表头位于第 {target_header_row} 行，来源表表头位于第 {source_header_row} 行。",
    )
